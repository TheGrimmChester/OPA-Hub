...
- Tokens are standard HS256 JWTs (`Open-Auth-Go`).
- **Co-deployed family** (`PEER_OAM_URL` set): **OAM** is the identity home (`iss=oam-api`). Hub **proxies** browser login/register/status to OAM; it does not mint a parallel user directory. Share `JWT_SECRET` with ORA/OSA/OPL/OPM so they validate the same OAM-issued tokens.
- **Solo OPA** (`PEER_OAM_URL` unset): hub issues tokens locally (`iss=opa-hub`). Edge agents do not issue family JWTs when hub-paired (`OPA_HUB_URL`); local agent login/directory CRUD returns `503 identity_home_oam`.
...
- Optional claim `project_ids` (allowlist) restricts non-admin users to those projects within their org. Hub middleware calls `EnforceProjectACL` after tenant header binding: wrong project → **403** (`project access denied`). Role **admin** bypasses the allowlist (lab seed `admin`/`admin` retains full `default-org` access). Admins may set `org_id` / `project_ids` on `POST /api/auth/register` (solo hub only).
...
- `POST /api/auth/register`: when OAM owns auth, registration is proxied to OAM. Solo hub self-reg caps at `viewer`. When `OPA_AUTH_REQUIRED=1`, registration requires an admin JWT; only admins may mint editor/admin roles.
...
- Agent registry list/get and `GET /api/tenancy/organizations` filter to the request scope when auth-enforced or headers are set. With `PEER_OAM_URL`, org picker reads the OAM directory.
- Alert / SLO / synthetics DELETE uses `OwnedRowPredicate` so cross-tenant deletes cannot target another org’s row by id alone.
- Unauthenticated → **401** when `OPA_AUTH_REQUIRED=1`. Wrong org with unbound JWT → that org’s data only (header-driven). JWT with bound `org_id` + wrong header → **403**. JWT with `project_ids` + non-member `X-Project-ID` → **403** (admins exempt).
